...
let filter p l =
  List.fold_left
    (fun acc ele -> if p ele then ele::acc else acc)
    []
    l;;

let partition p l =
  List.fold_right 
    (fun ele (left,right) -> if p ele then (ele::left, right) else (left, ele::right))
    l
    ([],[]);;
      
let rec sort = function
  | [] -> []
  | x::xs -> let (min, max) = partition (function y -> y < x) xs in
      sort min @ (x :: sort max);; 
